...
def undo_move():
    ...
    where = history.pop()
    if   where == 1: where = 2
    elif where == 2: where = 1
    elif where == 3: where = 4
    elif where == 4: where = 3
...
def print_scene():
    ...
    for i in range(100):
        for x in range(100):
            j = scene[i][x]
            if i == my_pos[0] and x==my_pos[1]: print("D", end='')
            elif j==1: print(' ', end='')
            elif j==0: print('#', end='')
            elif i%2==1 or x%2==1: print(' ', end='')
            elif j==-1: print('.', end='')
            elif j==2: print('O', end='')
        print()

set_report(my_pos, SPACE)
report = -1
...
G = nx.Graph()
while True:
    if tuple(my_pos) not in unexplored:
        unexplored[tuple(my_pos)] = [1,2,3,4]

    hints = 0
    while len(unexplored[tuple(my_pos)]) == 0:
        if len(history) != 0:
            undo_move()
            hints += 1
        else:
    ...
    nm = unexplored[tuple(my_pos)].pop()
    report = move(nm)
    history.append(nm)
    set_report(my_pos, report)
    if report == WALL: undo()
    else:
        G.add_edge(tuple(last()), tuple(my_pos))
    if report == OXIGEN:
        oxigeno = my_pos[:]

# Part 1
# Dijkstra to oxigen
print(nx.dijkstra_path_length(G, tuple([50,50]), tuple(oxigeno)))

# Part 2 
# Dijkstra from oxigen to all spaces points
# choose the farthest
a=[]
todos = []
for i in range(100):
    for j in range(100):
        if scene[i][j] == 1: 
            a = (i,j)
            todos.append(nx.dijkstra_path_length(G, tuple(oxigeno), a))
